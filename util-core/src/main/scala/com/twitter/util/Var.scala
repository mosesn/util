...
import scala.collection.{immutable, BuildFrom}
...
object Var {
  ...
  def collect[T: ClassTag, CC[X] <: Traversable[X]](
    vars: CC[Var[T]]
  )(implicit buildFrom: BuildFrom[CC[Var[T]], T, CC[T]]): Var[CC[T]] = {
    val vs = vars.toArray
    ...
    tree(0, vs.length).map { ts =>
      val b = buildFrom.newBuilder(vars)
      b ++= ts
      b.result()
    }
  }
  ...
  def collectIndependent[T: ClassTag, CC[X] <: Traversable[X]](
    ...
    implicit buildFrom: BuildFrom[CC[Var[T]], T, CC[T]]
  ): Var[CC[T]] =
    async(buildFrom.newBuilder(vars).result()) { v =>
      val N = vars.size
